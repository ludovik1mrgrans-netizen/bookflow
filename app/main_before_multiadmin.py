...
def seed():
    with Session(engine) as s:
        b = s.scalar(select(Business).where(Business.slug == "milo-grooming"))
        if not b:
...
def admin_ok(request: Request):
    return request.cookies.get("bookflow_admin") == os.getenv("ADMIN_KEY", "demo123")
...
def login(key: str = Form(...)):
    if key != os.getenv("ADMIN_KEY", "demo123"):
        return RedirectResponse("/admin/login?error=1", 303)
    r = RedirectResponse("/admin", 303); r.set_cookie("bookflow_admin", key, httponly=True, samesite="lax"); return r
